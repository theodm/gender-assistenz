...
from pgender.pipeline.correction.det.select_det_form import select_det_form
...
from pgender.wordlib2 import find_det, find_verb
...
def transform_noun(
    ...
    if existing_number == "Sing":
        ...
        # ___                                ___
        #
        for det in find_det(word):
            if det.text == "ihr":
                break

            new_det = select_det_form(det, existing_case, target_gender, target_number)
            new_det = preserve_case(new_det, det.text)
            new_det = transform_det(new_det, det.text)

            result.append({
                "from": det.idx,
                "to": det.idx + len(det.text) + (1 if len(new_det) == 0 else 0),
                "type": TYPE_DET,
                "replace_with": new_det
            })
...
def transform_noun_plural(
    ...
    return transform_noun(word, lex_new_word, _transform_noun, nothing, "Plur", "Fem")
...
def generate_possible_corrections_singular(word):
    ...
    possible_corrections.append({
            "type": "*",
            "changes": transform_noun(word, fnf[0], genderify_feminine_form, genderify_det_form, "Sing", "Fem")
    })
    ...
    possible_corrections.append({
            "type": "PLURAL_*",
            "changes": transform_noun(word, fnf[0], genderify_feminine_form, genderify_det_form, "Plur", "Fem")
    })
